...
class DrawPanel extends JPanel implements MouseListener {
    ...
    protected void paintComponent(Graphics g) {
        ...
        g.drawString("Cards Left: " + (52 - discard.size()), 147, 350);
    ...
    public void removeCard () {
        if (selected.size() == 2) {
            ...
            if (val1 + val2 == 11) {
                discard.add(selected.get(0).getSuit() + selected.get(0).getValue());
                discard.add(selected.get(1).getSuit() + selected.get(1).getValue());
                ArrayList<String> temp = new ArrayList<String>();
                temp = (ArrayList<String>) discard.clone();
                for (Card card: hand) {
        ...
        if (selected.size() == 3) {
            ...
            if (hasJack && hasKing && hasQueen) {
                discard.add(selected.get(0).getSuit() + selected.get(0).getValue());
                discard.add(selected.get(1).getSuit() + selected.get(1).getValue());
                discard.add(selected.get(2).getSuit() + selected.get(2).getValue());
                ArrayList<String> temp = new ArrayList<String>();
                temp = (ArrayList<String>) discard.clone();
                for (Card card: hand) {
                    temp.add(card.getSuit() + card.getValue());
